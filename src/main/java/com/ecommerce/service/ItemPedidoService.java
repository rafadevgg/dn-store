...
public class ItemPedidoService {
    ...
    public ItemPedidoResponseDto adicionar(ItemPedidoRequestDto dto) {
        PedidoModel pedido = pedidoRepository.findById(dto.cdPedido())
                .orElseThrow(() -> new ResourceNotFoundException("Pedido", "ID", dto.cdPedido()));

        ProdutoModel produto = produtoRepository.findById(dto.cdProduto())
                .orElseThrow(() -> new ResourceNotFoundException("Produto", "ID", dto.cdProduto()));

        if (!pedido.getStPedido().equals("PENDENTE")) {
            throw new BusinessException("Não é possível adicionar itens a um pedido já confirmado");
        }
    ...
    public ItemPedidoResponseDto buscarPorId(Long id) {
        ItemPedidoModel itemPedido = itemPedidoRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Item do pedido", "ID", id));
    ...
    public ItemPedidoResponseDto atualizar(Long id, ItemPedidoRequestDto dto) {
        ...
        if (!itemPedido.getPedido().getStPedido().equals("PENDENTE")) {
            throw new BusinessException("Não é possível alterar itens de um pedido já confirmado");
        }
    ...
    public void remover(Long id) {
        ...
        if (!itemPedido.getPedido().getStPedido().equals("PENDENTE")) {
            throw new BusinessException("Não é possível remover itens de um pedido já confirmado");
        }
    ...
    public void atualizarQuantidade(Long id, Integer novaQuantidade) {
        ...
        if (!itemPedido.getPedido().getStPedido().equals("PENDENTE")) {
            throw new BusinessException("Não é possível alterar a quantidade de itens de um pedido já confirmado");
        }

        if (novaQuantidade < 1) {
            throw new BusinessException("A quantidade mínima é 1");
        }
    ...
    private void atualizarTotalPedido(Long cdPedido) {
        PedidoModel pedido = pedidoRepository.findById(cdPedido)
                .orElseThrow(() -> new ResourceNotFoundException("Pedido", "ID", cdPedido));
